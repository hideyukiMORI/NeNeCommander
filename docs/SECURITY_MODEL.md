...
Inputs are length-bounded, parsed once, normalized without identity loss, and rejected when ambiguous. `FileSystemPath.Parse` bounds both raw input and the provider-normalized canonical result, so root separators and WSL alias expansion cannot produce an accepted path beyond the fixed boundary. Paths, distribution names, resource keys, settings values, and operation batches never enter shell or filesystem APIs as unchecked text. The Windows file launcher receives only `WindowsLocalPath`, passes its canonical text as one `ProcessStartInfo.FileName`, derives the working directory from its canonical parent, enables Shell association, and supplies no verb, arguments, or command interpreter. WSL discovery uses fixed process argument tokens, bounds both redirected streams before publishing output, and validates every reported distribution through the canonical path parser; one malformed line rejects the whole snapshot. WSL directory reads and mutations route only from validated `WslPath` values. Each untrusted enumerated name passes through `FileSystemPath.Child` before publication, and mutation targets are canonical children produced by the existing request model.
...
Preflight does not grant permanent trust. Adapters revalidate identity and containment at the side-effect boundary, treat links explicitly, use handles or provider identity where available, and report time-of-check/time-of-use changes without widening the target. Windows local identity combines the Win32 volume/file identifier obtained without following a reparse point with rewrite-sensitive metadata. Windows-side WSL identity (ADR-0049) combines the distribution name, the Linux inode read from a no-follow handle whose file system reports `9P`, kind, reparse tag, link count, length, last-write time, and kernel-assigned change time; the 9P namespace exposes no volume serial or 128-bit identifier, so the identity form is selected by the validated provider and never by a failed query. Entry identity defends against concurrent unprivileged replacement or rewrite. An actor who can mount inside a distribution (distribution root or the same user through FUSE) can place a different entry with an equal inode at the same path and is outside this defense, as is a clock rollback inside the distribution; hide accepted that scope on 2026-09-10. Query ambiguity or failure is closed. WSL mutations additionally revalidate the distribution and parent or destination boundary, refuse reparse points before tree copy and source deletion, preserve a failed copy's partial-target effect, and never fall back to a shell command. A user-requested Windows association handoff follows the separate path-activation contract in ADR-0046; it does not relax the filesystem snapshot, query, or mutation identity and containment requirements above. The launcher performs no existence, attribute, reparse, or identity preflight because such a probe cannot bind the later Shell resolution. It hands off the exact validated path once, reports only acceptance or a normalized rejection at that moment, and never widens, retries, or claims stable target identity.
