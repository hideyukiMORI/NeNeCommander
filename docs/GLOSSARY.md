...
| transient scope | One short-lived interaction the application session coordinates but does not own: the address editor and the command palette. Each is one closed state, one admission rule, and one expected-state validation, and each appears in `TransientScopeSnapshot`. |
| scope owner | The sole owner of one transient scope, in the `SettingsSession` shape: its own closed state under one lock, no reference to another owner, no freeze predicate of its own, no pane effect, and no dispatch. `AddressEditorSession` and `CommandPaletteSession` are the scope owners, carried together by `TransientScopeOwners` (ADR-0051). |
