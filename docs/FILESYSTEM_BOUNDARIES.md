...
The harness takes every identity from the production `WindowsWslFileSystem` on the entry's own path under ADR-0049, never from `WindowsFileIdentifier` directly, from enumeration, or from a runner-owned query; the 2026-09-07 attempt that stopped at the unsupported `FileIdInfo` query is superseded by that decision. Because a directory's `wsl-v2` identity changes whenever its direct children change, the harness re-captures an owned directory immediately after each mutation it performs and compares against that capture, while file and link identities stay stable across reads. A legacy file index with a zero volume serial is not accepted as a fallback. The live tier also proves read-only that an owned fixture's inode, link count, and change time equal a read-only `stat` inside the configured distribution, that a link and its target differ, and that an unchanged entry repeats one identity. The launcher requires the exact declared number of passing live cells; any skip or absence fails the tier.
