...
- In-progress vertical slice: [Issue #49](https://github.com/hideyukiMORI/NeNeCommander/issues/49) on branch `feat/49-hidden-item-visibility` (implementation checkpoint `dfe0593`), not merged: targeted tests and deep review are green; interactive-desktop screenshot and UI Automation proof are still owed because the machine is in active use
...
- Next vertical slice: finish `feat/49-hidden-item-visibility` — obtain the remaining non-destructive interactive-desktop screenshot and UI Automation proof for both visibility values without competing with hide's keyboard/focus. Keep its PR draft until that proof exists, then request the full CI gate under ADR-0025 (see the handoff). The hidden-visibility keystroke, which needs a keyboard-model revision, and the collision resolver (FS-007) follow.
