...
- In-progress vertical slice: [Issue #49](https://github.com/hideyukiMORI/NeNeCommander/issues/49) on branch `feat/49-hidden-item-visibility` (`74d9169`), gate green, not merged: deep review and runtime verification are still owed
- In-progress gate-performance work: [Issue #54](https://github.com/hideyukiMORI/NeNeCommander/issues/54) on branch `build/54-prune-proof-fixtures`; ADR-0026 centralizes generated-directory pruning without dropping current-tree inputs
- Policy foundation authorized by hide on: `2026-09-02`

## Current checkpoint

- Verified implementation baseline: `c3791ae07de9d77138c10e5d595922525c4e6665`
...
- Next vertical slice: finish `feat/49-hidden-item-visibility` — read the branch diff and ADR-0024, use targeted tests while finishing it, and verify both visibility values on the running app. Keep its PR draft until integration readiness, then request the full CI gate and complete the outstanding deep review under ADR-0025 (see the handoff). The hidden-visibility keystroke, which needs a keyboard-model revision, and the collision resolver (FS-007) follow.
