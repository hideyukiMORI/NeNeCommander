...
- Active hidden-item toggle work: [Issue #72](https://github.com/hideyukiMORI/NeNeCommander/issues/72); route `Ctrl+H` through the canonical key map to the active pane's session-only visibility transition.
- Policy foundation authorized by hide on: `2026-09-02`

## Current checkpoint

- Issues #89, #85, and #67 are integrated in that order. Issue #85's final candidate passed exact-head deep run `34017224394` and canonical Ready run `34018078267` before merge `fe745329`; live WSL proof remains a release environmental tier because `NENE_COMMANDER_WSL_TEST_ROOT` is unset and the repository has no executable live harness. Issue #67 then passed exact-head deep run `34018739071` and canonical Ready run `34019680542` before merge `1983ccc7`.
- Issue #72 is rebased onto `1983ccc71e7e304e07f480d66b38f200168eca6a`. Locked restore, a zero-warning Release build, Application 188/188, Presentation.WinUI 76/76, Architecture 5/5, 100% Application branch coverage, and 96.53% Presentation.WinUI branch coverage pass on that base. Commit validation, dependency review, and canonical Ready CI remain pending.

- Verified implementation baseline: `1983ccc71e7e304e07f480d66b38f200168eca6a`
...
- Next focused work: complete [Issue #72](https://github.com/hideyukiMORI/NeNeCommander/issues/72), then integrate the separately tracked collision resolver (#73) and settings persistence (#74). Cross-provider Windows local/WSL transfer remains a later provider-routing Issue and is not enabled by these changes.
