...
- Issue [#103](https://github.com/hideyukiMORI/NeNeCommander/issues/103) refines the sole path parser contract through ADR-0042: raw input and successful canonical text are each limited to 32767 UTF-16 code units. Its independent Draft, exact-head security deep review, and canonical Ready gate remain pending; Issue #99 does not carry a duplicate bookmark-local length rule.
...
- Draft Issue [#93](https://github.com/hideyukiMORI/NeNeCommander/issues/93) adds a fail-closed live WSL launcher, an owned-root harness, deterministic root-safety proof, and three required transfer assertions. Its first actual WSL attempt stopped before fixture or product mutation because `GetFileInformationByHandleEx(FileIdInfo)` returned `ERROR_NOT_SUPPORTED` for the WSL UNC directory. The sampled legacy query returned a zero volume serial and was rejected rather than weaken ADR-0033/0036 identity. Issue [#105](https://github.com/hideyukiMORI/NeNeCommander/issues/105) owns the compatibility blocker. All three live cells remain unexecuted, the retained dedicated root is empty, and #93 is not release proof or merge ready. The Windows high-contrast, DPI, narrow-width, eight-scheme, and keyboard-modal matrix remains unexecuted and is tracked by [Issue #94](https://github.com/hideyukiMORI/NeNeCommander/issues/94). Neither release tier is a passing result until its required cells run.
...
- Next focused work: implement the safe live-WSL harness and obtain the real-environment matrix tracked by [Issue #93](https://github.com/hideyukiMORI/NeNeCommander/issues/93), then complete the Windows UI release matrix tracked by [Issue #94](https://github.com/hideyukiMORI/NeNeCommander/issues/94). Cross-provider Windows local/WSL transfer remains a later provider-routing Issue and is not enabled by these changes.
