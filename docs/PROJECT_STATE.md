...
- Next vertical slice: hidden-item visibility — the persisted `showHiddenItems` reaches `PaneReducer` as the sole visible-set decision and hidden rows render with the hidden text brush (see the handoff). The collision resolver (FS-007) follows.
