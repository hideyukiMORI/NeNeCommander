...
- Next vertical slice: finish `feat/49-hidden-item-visibility` — read the branch diff and ADR-0024, run the canonical gate and the deep review, verify both visibility values on the running app, then open the pull request (see the handoff). The hidden-visibility keystroke, which needs a keyboard-model revision, and the collision resolver (FS-007) follow.
