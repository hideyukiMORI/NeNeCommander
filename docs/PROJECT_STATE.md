...
- Next vertical slice: the Direction C layout integration — tokens, pane header with pane number badge, 28 dip rows with kind icon and marker, one bottom bar with tone, key hints generated from the canonical key map (see the handoff). Feature work resumes with hidden-item visibility and the collision resolver (FS-007) afterwards.
