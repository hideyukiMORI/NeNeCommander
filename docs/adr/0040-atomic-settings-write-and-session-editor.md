...
- **The fixed temporary path has one-attempt ownership.** The adapter creates the sibling
  ...
  Cleanup revalidates both the parent chain and the temporary file's own identity. A detected
  ...
  identity is also revalidated before publish, so a detected replacement cannot become the
  settings document. The final `File.Delete` path reopen
...
- **The Windows local ancestor chain stays local.** The store accepts only a `WindowsLocalPath`;
  ...
  baseline is accepted only when both exact bytes and the stable provider identifier link the
  document back to this attempt's temporary entry. The chain is revalidated
...
- **One application session owns choices and ordered writes.** The settings session holds the
  ...
  evidence for its own revision but can never roll back a newer selected value. Shutdown awaits
  the queue.
...
ordered pending writes, old-completion non-reversal, success and failure persistence states, and
shutdown observation. Infrastructure tests prove exact serialization, absent move, valid atomic
...
cancellation, destination identity/content changes, all temporary entry kinds, reparse ancestors,
and parent replacement before and after temporary creation under ADV-012. Presentation and App-boundary tests
