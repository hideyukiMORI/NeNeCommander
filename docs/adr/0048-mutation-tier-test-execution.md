...
- **Each test project named by `eng/security-policy.json` `mutationProjects` additionally references `Microsoft.NET.Test.Sdk`**, pinned centrally in `Directory.Packages.props` at the version the pinned MSTest.Sdk itself resolves (18.9.0, unchanged by the 4.4.0 to 4.4.1 pin update), so the VSTest adapter and `testhost` are always present in the test output. `UseVSTest` is never set. The package graph and therefore every `packages.lock.json` are identical for the gate build and the mutation build, so `--locked-mode` restore and central transitive pinning cover the mutation run. Each of those test projects also lists `Microsoft.NET.Test.Sdk` in its `eng/architecture.json` CS-024 `packageReferences` allowlist; no other project may reference it.
...
## Pin review 2026-09-16 (Stryker.NET 5.0.0)

Reviewed under the migration clause when Dependabot proposed 5.0.0 (PR #128). Measured on main `f7d06c1` with identical mutant sets (Domain 245, Presentation.WinUI 1148): the 5.0.0 MTP runner now attributes coverage per test (its `coveredBy` distribution matches VsTest exactly and non-static status agreement is 99.6%), but it still does not isolate static mutants. Presentation static mutants score 13 Killed / 104 Survived under MTP `perTest` and 25 / 87 under the new `perTestInIsolation`, against 117 / 0 under VsTest; the trace log shows isolation applied only to the coverage-capture phase, never to mutant runs, and upstream PR #3695 and Issue #3742 remain open. The removal condition is therefore half met and this ADR stays in force: `test-runner: vstest`, the `Microsoft.NET.Test.Sdk` references, and the TST-008 checks are unchanged, and only the pinned version moves to 5.0.0, which requires the .NET 10 runtime the repository already pins. 5.0.0 VsTest reproduces the 4.16.0 verdicts on the same machine (Presentation status agreement 99.8%, `killedBy` agreement 100%); the two differences are Timeout reclassifications from the new dynamic timeout calculation, one of which surfaces a true survivor in `AsyncWorkOwner` and moves Presentation from 94.30% to 94.19%, still above its threshold. The new `coverage-analysis` option keeps its default `perTest`; `perTestInIsolation` isolates test runs during coverage capture, not mutant runs, and is not adopted. The next review happens at the next pin change.
